...
export default function Stock() {
  ...
  const handleCreateTransfer = async () => {
    try {
      ...
      // Special handling for shipments to central - requires superadmin confirmation
      const isShipmentToCentral = transferForm.toFermeId === 'centralE';
      ...
      const newTransfer: Partial<StockTransfer> = {
        ...
        status: isShipmentToCentral ? 'pending_approval' : 'pending', // Special status for central shipments
        ...
        ...(isShipmentToCentral && { requiresSuperadminApproval: true })
      };

      const transferDocRef = await addDoc(collection(db, 'stock_transfers'), newTransfer);

      // Create appropriate notification based on destination
      if (isShipmentToCentral) {
        // Create superadmin approval notification for central shipments
        const superadminNotification: Partial<TransferNotification> = {
          transferId: transferDocRef.id,
          type: 'central_shipment_approval',
          fromFermeId: sourceFermeId,
          fromFermeName: getFermeName(sourceFermeId),
          toFermeId: 'centralE',
          toFermeName: 'Centrale',
          item: sourceStock.item,
          quantity: parseInt(transferForm.quantity),
          unit: sourceStock.unit,
          message: `🏭 CONFIRMATION REQUISE: Envoi vers la centrale - ${sourceStock.item} (${transferForm.quantity} ${sourceStock.unit}) de ${getFermeName(sourceFermeId)}`,
          status: 'unread',
          createdAt: serverTimestamp(),
          targetAudience: 'superadmins', // Only for superadmins
          priority: transferForm.priority || 'high',
          requiresAction: true
        };

        await addDoc(collection(db, 'transfer_notifications'), superadminNotification);
      } else {
        // Standard notification for regular farm-to-farm transfers
      ...
      toast({
        title: "Succès",
        description: isShipmentToCentral
          ? `🏭 Envoi vers la centrale créé (N° ${trackingNumber}) - En attente d'approbation du superadmin`
          : `Transfert créé avec succès (N° ${trackingNumber})`
      });
    } catch (error) {
  ...
  // Superadmin approval functions for central shipments
  const handleApproveCentralShipment = async (transfer: StockTransfer) => {
    try {
      const transferRef = doc(db, 'stock_transfers', transfer.id);
      await updateDoc(transferRef, {
        status: 'pending', // Change to normal pending status after approval
        approvedAt: serverTimestamp(),
        approvedBy: user?.uid,
        approvedByName: user?.nom || user?.email
      });

      // Create notification for the sender about approval
      const approvalNotification: Partial<TransferNotification> = {
        transferId: transfer.id,
        type: 'central_shipment_approved',
        ...
        quantity: transfer.quantity,
        unit: transfer.unit,
        message: `✅ APPROUVÉ: Votre envoi vers la centrale a été approuvé par le superadmin - ${transfer.item} (${transfer.quantity} ${transfer.unit})`,
        status: 'unread',
        createdAt: serverTimestamp(),
        priority: 'medium'
      };

      await addDoc(collection(db, 'transfer_notifications'), approvalNotification);

      toast({
        title: "Envoi approuvé",
        description: "L'envoi vers la centrale a été approuvé avec succès"
      });
    } catch (error) {
      console.error('Erreur lors de l\'approbation:', error);
      toast({
        title: "Erreur",
        description: "Impossible d'approuver l'envoi",
        variant: "destructive"
      });
    }
  };

  const handleRejectCentralShipment = async (transfer: StockTransfer, reason: string) => {
    try {
      const transferRef = doc(db, 'stock_transfers', transfer.id);
      await updateDoc(transferRef, {
        status: 'rejected',
      ...
      const rejectionNotification: Partial<TransferNotification> = {
        transferId: transfer.id,
        type: 'central_shipment_rejected',
        fromFermeId: transfer.fromFermeId,
        fromFermeName: transfer.fromFermeName,
        toFermeId: 'centralE',
        toFermeName: 'Centrale',
        ...
        quantity: transfer.quantity,
        unit: transfer.unit,
        message: `❌ REJETÉ: Votre envoi vers la centrale a été rejeté - ${transfer.item} (${transfer.quantity} ${transfer.unit}). Raison: ${reason}`,
        status: 'unread',
        createdAt: serverTimestamp(),
        priority: 'high'
      };

      await addDoc(collection(db, 'transfer_notifications'), rejectionNotification);

      toast({
        title: "Envoi rejeté",
        description: "L'envoi vers la centrale a été rejeté"
      });
    } catch (error) {
      console.error('Erreur lors du rejet:', error);
      toast({
        title: "Erreur",
        description: "Impossible de rejeter l'envoi",
        variant: "destructive"
      });
    }
  };
  ...
  const getStatusBadgeVariant = (status: string) => {
    switch (status) {
      ...
      case 'pending_approval': return 'outline'; // Special styling for approval needed
                        ...
                        <TableRow key={transfer.id}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              ...
                              {/* Superadmin approval actions for central shipments */}
                              {transfer.status === 'pending_approval' && transfer.toFermeId === 'centralE' && isSuperAdmin && (
                                <>
                                  <Button
                                    size="sm"
                                    onClick={() => {
                                      setSelectedTransfer(transfer);
                                      setConfirmAction('approve_central');
                                      setShowConfirmDialog(true);
                                    }}
                                    className="bg-blue-600 hover:bg-blue-700"
                                    title="Approuver l'envoi vers la centrale"
                                  >
                                    <Check className="h-4 w-4" />
                                  </Button>
                                  <Button
                                    ...
                                    onClick={() => {
                                      setSelectedTransfer(transfer);
                                      setConfirmAction('reject_central');
                                      setShowConfirmDialog(true);
                                    }}
                                    title="Rejeter l'envoi vers la centrale"
                                  >
                                    <X className="h-4 w-4" />
                                  </Button>
                                </>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              ...
              {confirmAction === 'approve_central' && '🏭 Approuver l\'envoi vers la centrale'}
              {confirmAction === 'reject_central' && '🚫 Rejeter l\'envoi vers la centrale'}
            </AlertDialogTitle>
            <AlertDialogDescription>
              ...
              {confirmAction === 'approve_central' &&
                `Êtes-vous sûr de vouloir APPROUVER l'envoi de ${selectedTransfer?.quantity} ${selectedTransfer?.unit} de ${selectedTransfer?.item} de ${selectedTransfer?.fromFermeName} vers la centrale ?`
              }
              {confirmAction === 'reject_central' &&
                `Êtes-vous sûr de vouloir REJETER l'envoi de ${selectedTransfer?.quantity} ${selectedTransfer?.unit} de ${selectedTransfer?.item} de ${selectedTransfer?.fromFermeName} vers la centrale ?`
              }
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (selectedTransfer) {
                  ...
                  } else if (confirmAction === 'approve_central') {
                    handleApproveCentralShipment(selectedTransfer);
                  } else if (confirmAction === 'reject_central') {
                    handleRejectCentralShipment(selectedTransfer, 'Rejeté par le superadmin');
                  }
                }
              ...
              className={
                ...
                confirmAction === 'reject' || confirmAction === 'reject_central' ? 'bg-red-600 hover:bg-red-700' :
                confirmAction === 'approve_central' ? 'bg-blue-600 hover:bg-blue-700' :
            ...
            >
              ...
              {confirmAction === 'approve_central' && 'Approuver'}
              {confirmAction === 'reject_central' && 'Rejeter'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
